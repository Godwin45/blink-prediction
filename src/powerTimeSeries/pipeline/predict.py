...
class PredictionPipeline:
    ...
    def add_lags(self, df):
        target_map = df['WEIGHT'].to_dict()
        df['lag1'] = (df.index - pd.Timedelta('2 days')).map(target_map)
        df['lag2'] = (df.index - pd.Timedelta('10 days')).map(target_map)
        df['lag3'] = (df.index - pd.Timedelta('100 days')).map(target_map)
        return df
